...
- **Un administrateur déclenche les deux réveils de cron à la main** (`_shared/platformAuth.ts`). `scheduler` et `referee-tick` acceptent deux appelants : la clé de service, que présente `pg_cron`, et une session dont l'utilisateur figure dans `administrateurs`. L'identité vient de `/auth/v1/user` et de nulle part ailleurs ; l'appartenance se lit avec la clé de service, la table n'étant lisible que de ses membres ; un authentifié non administrateur est refusé **mot pour mot comme un inconnu**, la réponse n'apprenant pas que cette table existe. La réponse est le compte rendu du cron, tel quel.
  ...
  **`force` sort de la fenêtre du jeudi**, et rien d'autre : il ne déplace pas l'horloge. `scheduler` ouvre alors la vague du jeudi visé, la fait avancer et la clôt un autre jour — la vague créée porte le `debut` de ce jeudi-là, si bien que le vrai jeudi la retrouve ouverte au lieu d'en créer une seconde. `referee-tick` joue alors les parties de vague, au lieu des seules qualifications. **`force` est réservé aux administrateurs** : la clé de service seule ne l'obtient pas, un cron qui sortirait de sa fenêtre ne serait plus un cron hebdomadaire. Chaque déclenchement manuel est journalisé par la fonction et nommé dans son compte rendu (`declenchement`, `force`). L'écran « Mes IA » porte les deux commandes, visibles des seuls administrateurs.
