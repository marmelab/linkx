...
src/
  game/                 logique pure, sans React — voir src/game/CLAUDE.md
    ...
    openingBook.data.ts livre généré hors ligne à profondeur 7, 459 entrées
...
- Le livre d'ouverture (`openingBook.ts`) ne guide que le **maître**, sur son premier coup, et retombe sur la recherche en dehors de son périmètre. `openingBook.data.ts` est **généré** par `scripts/generate-opening-book.ts` à `--depth 7`, un palier au-delà de ce que le jeu atteint, en une à deux heures ; ne pas l'éditer à la main. Cette profondeur suit le moteur : à chaque fois qu'il gagne un palier en direct, le livre doit en gagner un aussi, sinon il ne fait plus que répéter ce que le jeu trouve seul. Il doit être engendré par le moteur courant : un livre hérité d'une autre évaluation affaiblit la recherche. Engendré par le moteur qui le lit, un palier au-dessus de ce que le jeu atteint, il la renforce — mesuré au budget du jeu : coup par coup, sur les 50 ouvertures, meilleur 30 fois, à égalité 20, jamais moins bon ; partie par partie, 34 victoires sur 48 contre 28 sans lui, soit 12 ouvertures gagnées contre 6 perdues (p = 0,24). Ses seules clés sont `white|`, l'ordinateur jouant toujours blanc : un duel qui alterne les couleurs le rend inerte une partie sur deux et ne peut donc pas le mesurer. C'est `scripts/duel-livre.ts` qui le mesure, apparié et à ouverture imposée, jamais `duel-maitre.ts`.
