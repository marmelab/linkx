...
- **Trois dépendances, et trois seulement, servent la plateforme de tournoi** : `react-router` (v7, `createHashRouter`), `@supabase/supabase-js` et `@tanstack/react-query` (v5). Elles ne sont **jamais** chargées par le jeu — `src/Root.tsx` les tient derrière un `React.lazy`, si bien qu'elles vivent dans un morceau à part et n'entrent pas dans le bundle d'entrée. Le routage est en **mode hash** parce que GitHub Pages sert des fichiers statiques : `/linkx/classement` rendrait une 404 au rechargement. La query string reste au jeu, dont `?moves=` ouvre une partie.
