...
supabase/               backend de la plateforme de tournoi, en Deno
  ...
  functions/
    _shared/            logique partagée, testée par le Vitest du dépôt
      ...
      essaiLocal.ts     deux entrées d'essai, fermées par défaut : horloge et adresses
...
- **Contrat des fonctions edge appelées par les écrans** : `register-bot` reçoit `{ name, url }`, `probe-bot` reçoit `{ url }`, `update-bot` reçoit `{ bot, status }` ; toutes répondent un JSON portant `ok`, `message`, et `field` sur un refus, que le formulaire affiche **sous le champ nommé**. `update-bot` est l'unique porte du statut d'une IA, `statut` étant réservé au service par un déclencheur : elle vérifie l'identité auprès de `/auth/v1/user`, n'agit que sur une IA de l'appelant, et n'ouvre que deux transitions — retirer une IA vivante, réactiver une IA en sommeil, qui repasse alors par la qualification en gardant l'Elo gelé. `retiree` est terminal, et rien ne mène à `active` : cela reste le verdict de l'ordonnanceur.
- **L'IA de la maison s'amorce à la main, une fois par projet, et rien ne marche avant.** Aucune migration ne la crée : `bots.proprietaire` référence `auth.users`, et `ia_maison` comme `statut` sont réservés au service par un déclencheur, donc hors de portée de `register-bot`. Tant qu'aucune ligne ne porte `ia_maison = true` **et** `statut = 'active'`, l'ordonnanceur rend « IA de la maison absente ou inactive » et **aucune IA ne peut se qualifier** : la plateforme est inerte. La marche à suivre, sur un projet neuf : ouvrir un compte pour elle par lien magique, déclarer l'IA par `register-bot` en donnant l'adresse publique de la fonction `bot-linkx`, puis, en SQL avec le rôle de service, `update public.bots set ia_maison = true, statut = 'active' where nom = '…'`. Le secret rendu par `register-bot` se dépose alors dans le secret `LINKX_BOT_SECRET` de la fonction `bot-linkx`, faute de quoi elle n'authentifie pas les appels de la plateforme.
- **La fenêtre du jeudi borne les vagues, pas les qualifications.** Le cron réveille les deux fonctions **chaque minute, tous les jours** (migration 20260904120000), et ce sont elles qui se gardent : hors fenêtre, `scheduler` n'ouvre ni ne classe aucune vague et se borne aux qualifications, `referee-tick` ne joue que les parties **sans vague**, qui sont les qualifications, et remet les autres en attente. Sans cela, une IA déclarée le lundi attendait le jeudi pour entrer au classement, quand l'histoire 14 la veut qualifiée pendant que son auteur regarde l'écran. C'est aussi ce qui a supprimé les entrées de cron « veille », qui rattrapaient à la main les deux heures de la vague tombant le mercredi UTC.
- **Deux entrées d'essai, fermées par défaut** (`_shared/essaiLocal.ts`), sans lesquelles la plateforme est inéprouvable en local — il n'y a ni jeudi à commande ni nom d'hôte public. `TOURNOI_ESSAI_HORLOGE=1` laisse `scheduler` et `referee-tick`, qui exigent déjà la clé de service, lire l'instant dans le corps de la requête ; `TOURNOI_ESSAI_ADRESSES` est une **liste blanche nominative** `{ hôte déclaré: adresse appelée }` qui dérive les seuls hôtes qui y figurent vers un service local, le contrôle d'adresse restant entier pour tous les autres. Elles ne se posent que par `supabase functions serve --env-file`, jamais sur un projet distant, et sans elles le code se comporte exactement comme en production.
